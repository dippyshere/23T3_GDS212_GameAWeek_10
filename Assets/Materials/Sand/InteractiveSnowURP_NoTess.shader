    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(Varyings IN) : SV_Target
            {
                ...
                // Mix realtime and baked GI (handles subtractive and shadowmask modes)
                // Save raw lightmap luminance before MixRealtimeAndBakedGI modifies bakedGI
                half rawBakedLum = Luminance(bakedGI);
                ...
                half4 extraColors;
                    ...
                    extraColors.rgb = litMainColors.rgb * mainLight.color.rgb * (subtractiveShadow + bakedGI);
                #else
                    float subtractiveShadow = shadow;
                    extraColors.rgb = litMainColors.rgb * mainLight.color.rgb * (shadow + bakedGI);
                #endif
                extraColors.a = 1;

                // colored shadows
                float3 coloredShadows = subtractiveShadow + lerp(_ShadowColor, 0, subtractiveShadow);
        ...
        Pass
        {
            ...
            MetaVaryings MetaVert(Attributes input)
            {
                ...
                #ifdef EDITOR_VISUALIZATION
                UnityEditorVizData(input.vertex.xyz, input.uv, input.staticLightmapUV, float2(0, 0), output.vizUV, output.lightCoord);
                #endif
                return output;
            }
